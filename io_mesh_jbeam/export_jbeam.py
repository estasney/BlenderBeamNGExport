    ...
    try:
        for objsel in selectedObjects:
            ...
            if '.jbeam' in objsel.name:
                ...
                filename = objsel.name + '.jbeam'
            print("File = " + str(filepath) + filename )
            ...
            if not(listbn):
                if(bpy.context.user_preferences.system.author == ""):
                    author = 'Export Jbeam' + ' v.' + __version__
                else:
                    author = bpy.context.user_preferences.system.author + "," + 'Export Jbeam' + ' v.' + __version__
                if '.jbeam' in objsel.name:
                ...
                for f in mesh.tessfaces:
                    ...
                    if len(vs) == 3:
                        ...
                        file.write('["%s","%s","%s"],\n' % (sortedNodes[nodeIndex1].nodeName, sortedNodes[nodeIndex2].nodeName, sortedNodes[nodeIndex3].nodeName))
                         ...
                         pass#self.report({'ERROR'}, 'ERROR: TriCol %i isn\'t tri' % vs.index)
                if not(listbn):
                    file.write('\t\t\t],\n')
            if not(listbn):
                file.write('\t}\n}')
